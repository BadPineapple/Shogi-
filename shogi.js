...
const startPieces = [
    lances, knights, silver, gold, king, gold, silver, knights, lances,
    "", bishop, "", "", "", "", "", rook, "",
    pawns, pawns, pawns, pawns, pawns, pawns, pawns, pawns, pawns,
    "", "", "", "", "", "", "", "", "",
    "", "", "", "", "", "", "", "", "",
    "", "", "", "", "", "", "", "", "",
    pawns, pawns, pawns, pawns, pawns, pawns, pawns, pawns, pawns,
...
function dragDrop (e){
    ...
    if (correctGo && valid){
        if (targetId <= 80 && targetId >= 54) {
            if (draggedElement.id == 'pawns' || draggedElement.id == 'lances' || draggedElement.id == 'knights' || draggedElement.id == 'silver') {
            ...
            if(taken)
                if(!beTaken){
                    ...
                    if (e.target.classList[1] === 'white')
                        wc.unshift(e.target.id)
                    else if(e.target.classList[1] === 'black')
                        bc.unshift(e.target.id)
                    console.log("wc " + wc)
                    console.log("bc " + bc)
...
function checkIfValid(target){
    ...
    switch (piece){
        ...
        case 'rook':
            if (targetId % 9 === startId || (targetId - (startId % 9)) +(startId % 9) === targetId && !(startId === targetId)) {
                if(targetId > startId){
        ...
        case 'knights':
            if (startId + width * 2 + 1 === targetId || startId + width * 2 - 1 === targetId && !(startId === targetId)) {
                return true
            }
            break;
        ...
        case 'dragon':
            if (targetId % 9 === startId || (targetId - (startId % 9)) +(startId % 9) === targetId || startId + width + 1 === targetId ||
              startId + width - 1 === targetId || startId - width + 1 === targetId || startId - width - 1 === targetId && !(startId === targetId)){
                if(targetId > startId){
                    for (i = startId + 9; i < targetId; i = i+ 9)
                        if (document.querySelector(`[square-id = "${i}"]`).firstChild?.classList.contains("piece"))
